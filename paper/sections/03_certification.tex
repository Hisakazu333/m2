...
deadline are accounted for, and a binary old-label declaration is produced by
that deadline (no abstention; any timeout is charged and mapped to an
arbitrary label).
\end{enumerate}
\end{assumption}
...
\begin{align}
 ...
 \label{eq:rates}
\end{align}
We use the convention that an empty minimum is $+\infty$: a universally valid
decoder ($B(a)=\varnothing$) has $d_i(a)=+\infty$.
...
\begin{table}[ht]
\centering\small\input{tables/analytic_table}
